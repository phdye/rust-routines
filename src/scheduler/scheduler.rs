...
pub struct Scheduler {
    ...
    /// Stealers for each worker's queue
...
impl Scheduler {
    ...
    pub fn new(config: SchedulerConfig) -> Result<Arc<Self>> {
        let scheduler = Arc::new_cyclic(|_weak_scheduler| {
            ...
            for i in 0..num_workers {
                ...
                // We'll start workers after construction
    ...
    fn start_workers(self: &Arc<Self>) -> Result<()> {
        ...
        for (_id, _worker) in workers.iter() {
            // We need to modify the worker to start it, but we have it behind Arc
            // This is a limitation of the current design - we'll need to refactor
            // For now, we'll skip starting workers in the constructor
        }
    ...
    pub fn schedule(&self, task: Task) -> Result<()> {
        ...
        // All workers busy, use round-robin distribution
        let num_workers = workers.len();
        if num_workers > 0 {
            let index = self.round_robin.fetch_add(1, Ordering::Relaxed) % num_workers;
        ...
        // No workers available, push to global queue
    ...
    pub fn shutdown(&self) -> Result<()> {
        ...
        for _worker in workers.values_mut() {
            // We need mutable access to stop workers
            // This is another design limitation we'll need to address
        }
